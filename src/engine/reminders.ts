...
export const REMINDER_COPY: Record<keyof Reminders, string> = {
...
export interface LocalPing {
  kind: keyof Reminders;
  /** minutes since local midnight */
  minutes: number;
  /** local weekdays it fires on (0-6); all 7 for daily kinds */
  days: number[];
}
...
/** Expand the schedule into concrete local ping times. */
export function localPings(r: Reminders): LocalPing[] {
 ...
 */
export function slotsFor(r: Reminders, tzOffsetMinutes: number): number[] {
  const slots = new Set<number>();
  for (const ping of localPings(r)) {
 ...
 */
export function duePingsToday(r: Reminders, weekday: number, nowMinutes: number): LocalPing[] {
  return localPings(r).filter((p) => p.days.includes(weekday) && p.minutes <= nowMinutes);
}
